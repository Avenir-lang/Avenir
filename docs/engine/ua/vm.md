...
Значення структур, викинуті через `OpThrow`, передаються в catch-обробники
без обгортання, що дозволяє типізованим catch-клаузам зіставляти за
типом структури.

`OpIsStructType` перевіряє вершину стека і кладе `true`/`false` залежно
від збігу з вказаним індексом типу структури. Це використовується
в ланцюжку диспатчу типізованих catch-клауз.
