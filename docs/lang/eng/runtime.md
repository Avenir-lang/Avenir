...
Avenir supports async function execution with `async` and `await`.

- Async calls return a future runtime value.
- `await` reads the result of that future.
- Runtime uses a single-threaded scheduler and event loop.
- Pending futures suspend the current task and resume it when ready.
