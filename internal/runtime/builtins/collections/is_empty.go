...
func init() {
	builtins.Register(builtins.Builtin{
		...
		Call: func(env builtins.Env, args []interface{}) (interface{}, error) {
			if len(args) != 1 {
				return value.Value{}, fmt.Errorf("list.isEmpty expects 1 argument (receiver), got %d", len(args))
			}
			receiver := args[0].(value.Value)

			if receiver.Kind != value.KindList {
				return value.Value{}, fmt.Errorf("list.isEmpty called on non-list type %v", receiver.Kind)
			}

			return value.Bool(len(receiver.List) == 0), nil
		},
	})
}
